...
import { SubCategoryGetRespons } from '../../../lib/DataSet/MarketPlase'
import moment from 'moment'
import { SubCategoryChangeStatus, SubCategoryGet } from '../../../Service/MarketPlase'
...
type Props = {}
type SubCategoryGetResponsTable = SubCategoryGetRespons & { index: number, delete?: boolean }
...
export const SubCategory: React.FC<Props> = () => {
    ...
    const [isModalId, setIsModalId] = useState<SubCategoryGetResponsTable | undefined>();
    ...
    const makeSubCatagoryTableData = (item: SubCategoryGetResponsTable) => {
        return {
            sl: `#${item.index + 1}`,
            name: item.name,
            description: item.description,
            UnderCategory: item.categoryData.name,
            ...
            action: (
                <div style={{ justifyContent: 'space-between' }}>
                    ...
                    <button
                        ...
                        onClick={() => {
                            setEditData({ description: item.description, name: item.name, _id: item._id, categoryID: item.categoryData._id })
                            setIsEdit(true)
    ...
    const SubcatagoryDelete = (item: SubCategoryGetResponsTable | undefined) => {
        if (item) {
            SubCategoryChangeStatus(item._id, { isDeleted: true }).then((data) => {
                if (data.status) {
    ...
    const catagoryChangeStatus = (item: SubCategoryGetResponsTable | undefined) => {
        if (item) {
            SubCategoryChangeStatus(item._id, { status: !item.status }).then((data) => {
                if (data.status) {
                    catagoryData[item.index] = makeSubCatagoryTableData({ ...item, status: !item.status })
                    setCatagoryData(catagoryData);
                    setRefress(!refress)
                    setIsModalOpen(false)
                    toast(data.massage, { type: toast.TYPE.SUCCESS });
    ...
    useEffect(() => {
        SubCategoryGet(page).then((data) => {
            if (data.status && data?.data && data.dataLimit) {
                let newArr: any[] = data.data.map((it, index) => makeSubCatagoryTableData({ ...it, index }))
